//
//  CommentForWineViewController.m
//  食全酒美
//
...
@interface CommentForWineViewController ()

@end

@implementation CommentForWineViewController
@synthesize mWineId;
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    [dataHandler WineCommentListRecord:mWineId :mPagnum*10 :(mPagnum+1)*10 :self];
...
{
    if ([mCommentArray count]>0 ){
        static NSString * tempCommentForWineCell = @"CommentForWineCell";
        CommentForWineCell *cell = nil;//(CommentForWineCell*)[tableView dequeueReusableCellWithIdentifier:tempCommentForWineCell];
        if (cell == nil) {
            NSArray *nib = [[NSBundle mainBundle]loadNibNamed:@"CommentForWineCell" owner:self options:nil];
